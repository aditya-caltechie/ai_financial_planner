...
## Clear Step-by-Step Summary

### Final Architecture of Alex's Data Pipeline

They are now completing the **automated data ingestion loop** with this full flow:

1. **EventBridge Scheduler (cron job)**  
   Runs automatically every 2 hours.

2. **Alex Scheduler Lambda**  
   - Triggered by EventBridge.  
   - Its only job is to start the Researcher (by calling App Runner).

3. **App Runner (container)**  
   Runs the Researcher Agent.

4. **Researcher Agent**  
   - Uses Bedrock (or OSS model) as its LLM.  
   - Uses MCP Server + Playwright to browse the internet and conduct financial research.  
   - Once it gathers useful information, it calls the **Ingest API** (via API Gateway with security key).

5. **Ingest Lambda function**  
   - Receives the research data.  
   - Calls the SageMaker endpoint (**all-MiniLM-L6-v2**).  
   - Converts the text into a vector embedding.

6. **S3 Vectors**  
   Stores the final vector in the vector database.

### Overall Purpose

Every 2 hours, Alex automatically researches the web, turns the new findings into vectors, and adds them to its memory — creating a continuous, automated RAG knowledge base.

This is the complete, production-like automated data pipeline for the Alex financial planner project.
